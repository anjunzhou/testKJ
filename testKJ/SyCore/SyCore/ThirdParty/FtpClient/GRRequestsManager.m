...
- (void)_processNextRequest:(SySendData *)sendData;
...
- (void)startProcessingRequests
{
    if (_isRunning == NO) {
        _isRunning = YES;
        [self _processNextRequest:nil];
    }
}

- (void)startProcessingRequests:(SySendData *)sendData
{
    if (_isRunning == NO) {
        _isRunning = YES;
        [self _processNextRequest:sendData];
    }
}
...
-(GRRequest *)getGRRequest{
    return self.currentRequest;
}

-(NSMutableDictionary *)getGRRequestStoreDic{
    return self.currentRequest.storeDic;
}
...
- (void)requestCompleted:(GRRequest *)request
{
    ...
    [self _processNextRequest:[request.storeDic objectForKey:kSendData]];
}
...
- (void)_processNextRequest:(SySendData *)sendData
{
    ...
    if(sendData){
        [self.currentRequest.storeDic setValue:sendData forKey:kSendData];
    }
