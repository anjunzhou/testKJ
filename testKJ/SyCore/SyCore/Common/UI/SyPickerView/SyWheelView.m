...
#import "SyWheelView.h"
#import <QuartzCore/QuartzCore.h>

@implementation SyWheelView
...
- (void)initialize{
    ...
    self.unknownParam1 = self.unknownParam2 = 2.0;
    ...
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapwheel:)];
...
- (void)selectedRowInComponent:(NSInteger)component{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        toDescelerate = YES;
        [self animateViewsWithVelocity:[NSNumber numberWithDouble:5]];
    }];
...
- (void)layoutView:(UIView *)__view atIndex:(int)index animated:(BOOL)animated duration:(double)duration{
    ...
    const double dAngle = M_PI / _currentNumber;
    ...
    angle=(index - _currentIndex) * dAngle;
    
    if (fabs(angle)<M_PI/2.6) {
        __view.hidden=NO;
    }
    else{
    ...
    double z = -self.bounds.size.height / _unknownParam1 + self.bounds.size.height / _unknownParam1 * cos(fabs(angle));
    
    //y就是y坐标；当然也要算出来view 在y轴上距离圆点的距离 才能看起来像是球面效果
    double y = 0.0;
    y = self.bounds.size.height / _unknownParam2 + self.bounds.size.height / _unknownParam1 * sin(angle);
    
    void(^changeLayerLayout)(void) = ^(void) {
        __view.layer.position = CGPointMake(__view.layer.position.x, y);
        __view.layer.zPosition = z;
        __view.layer.transform=CATransform3DMakeRotation(-angle, 0.01, 0.0, 0.0);
...
- (void)layoutView:(UIView *)__view byAngle:(double)angle animated:(BOOL)animated duration:(double)duration{
    ...
    double z = -self.bounds.size.height / _unknownParam1 + self.bounds.size.height / _unknownParam1 * cos(fabs(_angle));
    
    
    double y = self.bounds.size.height / _unknownParam2 + self.bounds.size.height / _unknownParam1 * sin(_angle);
...
#pragma mark 点击事件
-(void)tapwheel:(UITapGestureRecognizer *)gesture{
    
    toDescelerate = NO;;
    CGPoint point = [gesture locationInView:self];
    //    NSLog(@"handleSingleTap!pointx:%f,y:%f",point.x,point.y);
    int heighthalf = self.bounds.size.height / 2;
    int velocityY = heighthalf - point.y;
    if ([_views count] > 0) {
        [UIView animateWithDuration:0.2 animations:^{
            [self animateViewsWithVelocity:[NSNumber numberWithDouble:velocityY * 100]];
        } completion:^(BOOL finished) {
            toDescelerate = YES;
            [self animateViewsWithVelocity:[NSNumber numberWithDouble:5]];
        }];
...
- (void)animateViewsWithVelocity:(NSNumber *)velocity{
    ...
    if ([velocity doubleValue] != 0) {
        ...
        double _dAngle = [velocity doubleValue] * M_PI / _currentNumber / self.bounds.size.height / 10;
